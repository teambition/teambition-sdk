...
export class TasklistModel extends BaseModel<Tasklist> {
  private _schemaName = 'Tasklist'

  addTasklists(_projectId: string, tasklists: Tasklist[]): Observable<Tasklist[]> {
    const result = datasToSchemas<Tasklist, Tasklist>(tasklists, Tasklist)
  ...
  add(tasklist: Tasklist): Observable<Tasklist> {
    const result = dataToSchema<Tasklist, Tasklist>(tasklist, Tasklist)
